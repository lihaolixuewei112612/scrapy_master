...
def request_header():
    headers = {
        ...
        'User-Agent': UserAgent(use_cache_server=False).Chrome #谷歌浏览器
    }
    return headers
...
#发送请求，获得响应
def send_request(book_tag):
    #爬取7页，可自行修改
    for i in range(1,8):
        print(f'正在抓取第{i}页……')
        response = requests.get(url=f'http://www.ip3366.net/free/?stype=1&page={i}', headers=request_header())
        ...
        for td in tr_list:
            ...
            bac=do_spider(book_tag,proxy)      #开始检测获取到的ip是否可以使用
...
def book_spider(book_tag,proxy):
    page_num = 0;
    book_list = []
    try_times = 0
    proxies = {
    ...
    while (1):
        ...
        try:
            ...
            source_code = opener.open(req).read().decode('utf-8')
...
def do_spider(book_tag_lists,proxy):
    book_lists = []
    for book_tag in book_tag_lists:
        book_list = book_spider(book_tag,proxy)
